...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (isProvider) {
        toast.success("Codsigaaga waa la gudbiyey!");
      ...
      const serverMessage = error.response?.data?.message || "Cillad farsamo ayaa dhacday";
      ...
      <div className="max-w-[600px] w-full bg-white/80 backdrop-blur-xl rounded-[3.5rem] shadow-[0_40px_100px_rgba(186,230,253,0.3)] border border-white p-10 md:p-14 relative">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="relative group">
            <FiBriefcase className="absolute left-5 top-5 text-slate-300 group-focus-within:text-sky-500" size={18} />
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="relative group">
              ...
              {errors.name && <p className="text-rose-500 text-[10px] font-black mt-2 ml-4">{errors.name.message}</p>}
            </div>
            <div className="relative group">
              ...
              {errors.phone && <p className="text-rose-500 text-[10px] font-black mt-2 ml-4">{errors.phone.message}</p>}
            </div>
          </div>
          ...
          <div className="relative group">
            ...
            {errors.email && <p className="text-rose-500 text-[10px] font-black mt-2 ml-4">{errors.email.message}</p>}
          </div>

          <div className="relative group">
            <FiMapPin className="absolute left-5 top-5 text-slate-300 group-focus-within:text-sky-500" size={18} />
            <input {...register('location')} placeholder="Location (City)" className="w-full pl-14 pr-4 py-5 bg-slate-50 rounded-[1.8rem] font-bold border-none focus:ring-4 ring-sky-100 transition-all" />
            {errors.location && <p className="text-rose-500 text-[10px] font-black mt-2 ml-4">{errors.location.message}</p>}
          </div>
          ...
          <button 
            ...
            className="w-full bg-sky-500 text-white py-6 rounded-[2rem] font-black uppercase tracking-[0.4em] text-[10px] hover:bg-slate-900 transition-all shadow-xl active:scale-95 disabled:opacity-50 mt-4"
          >
        ...
        <div className="mt-10 text-center">
          <p className="text-slate-400 font-bold text-sm">
            ...
            <Link 
              to="/login" 
              className="text-sky-500 hover:text-slate-900 transition-colors ml-2 underline underline-offset-8 decoration-2 decoration-sky-100 hover:decoration-sky-500"
            >
              Sign In
            </Link>
          </p>
